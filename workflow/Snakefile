...
if config["sector"]["district_heating"]["add_subnodes"] and config["sector"][
    "district_heating"
].get("add_subnodes", True):
    ...
    use rule build_temperature_profiles from pypsaeur with:
        params:
            **{k: v for k,v in rules.build_temperature_profiles.params.items()},
        input:
            **{
                k: v
                for k, v in rules.build_temperature_profiles.input.items()
                if k != "regions_onshore"
            },
            regions_onshore=resources(
                "regions_onshore_base-extended_s_{clusters}.geojson"
            ),

    use rule build_central_heating_temperature_profiles from pypsaeur with:
        params:
            **{k: v for k,v in rules.build_central_heating_temperature_profiles.params.items()},
        input:
            **{
                k: v
                for k, v in rules.build_central_heating_temperature_profiles.input.items()
                if k != "regions_onshore"
            },
            regions_onshore=resources(
                "regions_onshore_base-extended_s_{clusters}.geojson"
    ...
    use rule build_cop_profiles from pypsaeur with:
        input:
            **{
                k: v
                for k, v in rules.build_cop_profiles.input.items()
                if k != "regions_onshore"
            },
            regions_onshore=resources(
                "regions_onshore_base-extended_s_{clusters}.geojson"
    ...
    use rule build_heat_source_potentials from pypsaeur with:
        input:
            **{
                k: v
                for k, v in rules.build_heat_source_potentials.input.items()
                if k != "regions_onshore"
            },
            regions_onshore=resources(
                "regions_onshore_base-restricted_s_{clusters}.geojson"
